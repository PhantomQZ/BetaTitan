...
@-webkit-keyframes hue {
	...
	function block_id(id)
	{
     ...
     {
        window.location.href='DeveloperPageGame.php?block_id='+id;
     }
	}
	function unblock_id(id)
	{
     ...
     {
        window.location.href='DeveloperPageGame.php?unblock_id='+id;
     }
	}
	function gothr()
	{
	...
	<div id="topdiv">
		<div id="topdivHeader">
			...
			<ul class="tablist"> 
				...
				<li><a href="DeveloperPageMember.php?dev=<?php echo $devid;?>">Members</a></li> 
				<li id="current"><a href="DeveloperPageGame.php?dev=<?php echo $devid;?>">Games</a></li> 
				<?php 
			...
			<div id="SummaryDescrip"><!--Game List-->
			<table border = "2">
			<?php 
				$x = 0;
				$sql = "select Dev_Group_Name from developer_group where Dev_ID = '$devid'";
				$first = mysqli_fetch_assoc(mysqli_query($conn,$sql));
				$dev_name = $first['Dev_Group_Name'];
				$sql2 = "select * from game where Dev_Group_Name = '$dev_name'";
				$second = mysqli_query($conn,$sql2);
				if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true)
				{
						$user_check = $_SESSION['login_user'];   
							$result = mysqli_query($conn, "select * from user where User_usrname LIKE '$user_check' ");
				...
				if($adcheck == '1' && $did == $devid)
					{
						...
						{
							$Name = $third['Dev_Group_Name'];
							$id = $third['Game_Register_ID'];
							?>
							<tr>
								<td><?php echo $Name; ?></td>
								<td>
								...
								else if($third['Approve']==1)
								{echo "Approved";}
								else if($third['Approve']==2)
								{echo "Rejected";}
								else if($third['Approve']==3)
								{echo "Blocked";}
								?>
								</td>
							<?php 
							...
							{
								?>
									<td><a href="javascript:block_id(<?php echo $third['Game_Register_ID'];?>)">Block</a></td>
								<?php
							}
							else if($third['Approve']>=3)
							{
								?>
									<td><a href="javascript:unblock_id(<?php echo $third['Game_Register_ID'];?>)">Unblock</a></td>
								<?php
							}
							?></tr><?php
							$x++;
					...
					else
				{
					...
					<?php
					while($four = mysqli_fetch_assoc($second))
					{
						$Name2 = $four['Dev_Group_Name'];
						?>
						<tr>
							<td><?php echo $Name2; ?>
						</tr>
            ...
            <br>
			<input type="button" onclick="gothr()" value="Register New Game">
			</div><!--Summary Description-->
			<div id="CommentSec">
			<h1>Comments</h1>
			<div style="border:1px solid black;width:580px;height:200px;overflow:scroll;">
			<?php
			$cmt = mysqli_query($conn, "select * from dev_comment where Dev_Group_Name='$name'");
