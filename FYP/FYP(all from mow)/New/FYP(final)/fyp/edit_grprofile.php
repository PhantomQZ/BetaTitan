				...
				<div id="login">
					<span>
						<?php
							...
							$first = mysqli_query($conn, "select * from developer_group where Dev_ID = '$devid'");
...
<?php 
	...
	{
		$gname = $cname = $cadd = $state = $postcode = $country = $phone = $fax = $gemail = $homepage = $summary = "";
		
		$gname = test($_POST['GroupName']);
		$privacy = $_POST['privacy'];
		$gemail = test($_POST['grpemail']);
		$company = $_POST['Company'];
		$cname = test($_POST['CName']);
		$cadd = test($_POST['CAddress']);
		$state = test($_POST['State']);
		$postcode = test($_POST['Postcode']);
		$country = test($_POST['Country']);
		$phone = test($_POST['CPhone']);
		$fax = test($_POST['CFax']);
