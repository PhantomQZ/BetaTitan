...
$results = $conn->query("SELECT Game_name, Game_prequel, Game_Register_ID, Game_boxshot, Game_price, Game_homepage FROM game where Approve = '1'");
...
<br>
		<p style="color:white;">Search Game:<input type="text" name="keyword" size="20">
...
if(isset($_POST["search"]))
{
	...
	$result2 = mysqli_query($conn,"select * from game where Game_name like '%$keyword%' and Approve = '1'");
	if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true)
	{
while($row = $result2->fetch_assoc()) {
$picture = $row['Game_boxshot'];
$link = $row['Game_Register_ID'];
$game .= <<<EOT
<li>
<form class="form-item">
<h4>{$row["Game_name"]}</h4>
<div><a href="gamepage.php?gameid=$link"><img src="$picture" width="150px"; height="150px";"></a></div>
...
<form class="form-item">
<h4>{$row["Game_name"]}</h4>
<div id="itcolor"><a href="gamepage.php?gameid=$link"><img src="$picture" width="150px"; height="150px";"></a></div>
<div>Price : {$currency} {$row["Game_price"]}<div>
<div class="item-box">
    <input name="Game_Register_ID" type="hidden" value="{$row["Game_Register_ID"]}">
	<button type="button" onclick="alert('You must login to purchase')">Add to Cart</button>
