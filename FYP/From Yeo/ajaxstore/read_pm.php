...
$req2 = mysqli_query($conn, "select pm.timestamp, pm.message, user.User_id as userid, user.User_usrname from pm, user where pm.id='$id' and user.User_ID='pm.user1' order by pm.id2");
$dn2 = mysqli_fetch_array($req2);
...
if(isset($_POST['message']) and $_POST['message']!='')
{
	...
	if(mysqli_query($conn, 'insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "'.(intval(mysql_num_rows($dn2))+1).'", "", "'.$_SESSION['userid'].'", "", "'.$message.'", "'.time().'", "", "")') and mysqli_query($conn, 'update pm set user'.$user_partic.'read="yes" where id="'.$id.'" and id2="1"'))
	{
?>
...
if($dn2['User_usrname']!='')
{
	echo '<img src="'.htmlentities($dn2['User_usrname']).'" alt="Image Perso" style="max-width:100px;max-height:100px;" />';
}
